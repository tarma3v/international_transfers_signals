...
def _position_in_range(past: np.ndarray, window: int) -> float:
    ...
    Это НЕ перцентиль: чувствительно к одному выбросу на границе окна.
    Настоящий перцентиль считает `_share_of_days_beaten` — он нужен для текста
...
def _share_of_days_beaten(past: np.ndarray, window: int) -> float:
    """Доля дней окна, которые сегодняшний курс ПОБИВАЕТ, 0..100 — честный перцентиль.
