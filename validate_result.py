...
from ml.targets import benefit_backward_only, benefit_forward_only, build_targets
...
RATE = 0.19  # переопределяется в main() по периоду разработки


def oos_scores(X, y, dates, h):
    folds = walk_forward_folds(dates, 2021, h)
...
def main() -> None:
    ...
    global RATE
    RATE = reference_rate(
        BASELINES["ТЗ: уровень (нижний дециль)"](X[:, :-1], names), dates, 2021)
    ...
    oos, sc, fr = oos_scores(X, y, dates, H)
    ...
    print("=" * 104)
    print(f"{'правило / модель':<34}{'выгода ±h':>12}{'форвард':>11}{'95% ДИ форв.':>19}{'обратная':>12}{'доля обр.':>11}")
    ...
    for nm, f in fired_map.items():
        tot = np.nanmean(np.where(f, (fwd + bwd) / 2, np.nan))
        a = np.nanmean(np.where(f, fwd, np.nan))
        b = np.nanmean(np.where(f, bwd, np.nan))
        lo, hi = bootstrap_ci(fwd[f & ~np.isnan(fwd)])
        share = abs(b) / (abs(a) + abs(b)) * 100 if (abs(a) + abs(b)) > 0 else 0
        print(f"{nm:<34}{tot:>+11.0f}бп{a:>+10.0f}бп{f'[{lo:+.0f}; {hi:+.0f}]':>19}{b:>+11.0f}бп{share:>10.0f}%")
    ...
    def null_band(n_fire: int) -> tuple[float, float, float]:
        ...
        С честным порогом модели срабатывают с разной частотой, а ширина полосы
        зависит от числа сработавших дней. Одна общая полоса на 19 % занижала бы
        порог значимости для редких моделей.
        """
        null = []
        for _ in range(1000):
            picked: list[int] = []
            while len(picked) < n_fire:
                st_ = rng.integers(0, len(idx_ok))
                picked += list(idx_ok[st_ : st_ + 3])
            null.append(y[np.array(picked[:n_fire])].mean() / base)
        null = np.sort(null)
    ...
    for nm, f in fired_map.items():
        ...
        for r in np.where(f & ~np.isnan(y))[0]:
            ...
            prev = [
                y[q]
                for q in order
                if dates[q] < d0 and dates[q] >= d0 - dt.timedelta(days=90) and not np.isnan(y[q])
            ]
            if prev:
