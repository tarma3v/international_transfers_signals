...
минимумом и максимумом. Текст называет **перцентиль** — долю дней, которые
сегодняшний курс побивает. На выбросах они расходятся: один старый пик на краю
окна сдвигает диапазон целиком, не трогая перцентиль.

Разрешённая кейсом формулировка «выгоднее, чем в 85 % дней» обещает именно
перцентиль. Считать одно, а писать про другое нельзя — поэтому в коде это два
...
`pct_range_90 >= 95` срабатывает на 99-м перцентиле квартального диапазона —
...
между пушем и экраном перевода перестаёт быть нормой и становится остаточным случаем —
...
| Решение | Почему так |
|---|---|
| Заголовок повторяет пуш дословно | Клиент должен увидеть, что попал туда, куда его звали. Расхождение — частая причина отказа на этом экране |
...
| **Частота уровня названа заранее** | «Столько давали 14 дней из последних девяноста — примерно раз в неделю». Калибровка ожидания, снова фактом о прошлом |
...
| Компонент | Механизм ценности | Нужен прогноз | Что измеряет пилот |
|---|---|---|---|
