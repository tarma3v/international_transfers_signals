"""Packet-CN: causal shadow-rate signals from the National Bank of Kyrgyzstan."""
...
from research.round6_kyrgyz_central_bank_features import (
    build_kyrgyz_nbkr_features, causality_check, load_kyrgyz_nbkr,
)


OUT = Path("results/research/round6/kyrgyz_central_bank_models")
...
RAW_ORDER = (
    "kyrgyz_nbkr_direct_basis", "kyrgyz_nbkr_negative_direct_basis",
    "kyrgyz_nbkr_usd_basis", "kyrgyz_nbkr_negative_usd_basis",
    "kyrgyz_nbkr_cny_basis", "kyrgyz_nbkr_negative_cny_basis",
    "kyrgyz_nbkr_consensus_basis", "kyrgyz_nbkr_negative_consensus_basis",
    "kyrgyz_nbkr_inverse_rub_momentum_1", "kyrgyz_nbkr_inverse_rub_momentum_2",
    "kyrgyz_nbkr_inverse_rub_momentum_5", "kyrgyz_nbkr_inverse_rub_momentum_10",
    "kyrgyz_nbkr_direct_usd_disagreement", "kyrgyz_nbkr_direct_cny_disagreement",
    "kyrgyz_nbkr_usd_cny_disagreement", "kyrgyz_nbkr_consensus_stale20",
)
...
def main() -> None:
    ...
    local, digest = load_kyrgyz_nbkr()
    matrix, names = build_kyrgyz_nbkr_features(index, series, references, local)
    causality_check(index, series, references, local)
    col = {name: matrix[:, names.index(name)].astype(float) for name in names}
    raw = {
        "kyrgyz_nbkr_direct_basis": col["kyrgyz_nbkr_direct_basis_bps"],
        "kyrgyz_nbkr_negative_direct_basis": -col["kyrgyz_nbkr_direct_basis_bps"],
        "kyrgyz_nbkr_usd_basis": col["kyrgyz_nbkr_usd_basis_bps"],
        "kyrgyz_nbkr_negative_usd_basis": -col["kyrgyz_nbkr_usd_basis_bps"],
        "kyrgyz_nbkr_cny_basis": col["kyrgyz_nbkr_cny_basis_bps"],
        "kyrgyz_nbkr_negative_cny_basis": -col["kyrgyz_nbkr_cny_basis_bps"],
        "kyrgyz_nbkr_consensus_basis": col["kyrgyz_nbkr_consensus_basis_bps"],
        "kyrgyz_nbkr_negative_consensus_basis": -col["kyrgyz_nbkr_consensus_basis_bps"],
        "kyrgyz_nbkr_inverse_rub_momentum_1": -col["kyrgyz_nbkr_rub_quote_ret_1"],
        "kyrgyz_nbkr_inverse_rub_momentum_2": -col["kyrgyz_nbkr_rub_quote_ret_2"],
        "kyrgyz_nbkr_inverse_rub_momentum_5": -col["kyrgyz_nbkr_rub_quote_ret_5"],
        "kyrgyz_nbkr_inverse_rub_momentum_10": -col["kyrgyz_nbkr_rub_quote_ret_10"],
        "kyrgyz_nbkr_direct_usd_disagreement": col["kyrgyz_nbkr_direct_minus_usd_bps"],
        "kyrgyz_nbkr_direct_cny_disagreement": col["kyrgyz_nbkr_direct_minus_cny_bps"],
        "kyrgyz_nbkr_usd_cny_disagreement": col["kyrgyz_nbkr_usd_minus_cny_bps"],
        "kyrgyz_nbkr_consensus_stale20": delayed_by_currency(
            col["kyrgyz_nbkr_consensus_basis_bps"][:, None], index, rows=20,
        )[:, 0],
    }
    ...
    finalists = {"incumbent": incumbent, "kyrgyz_selected": raw_outputs[selected_raw]}
    for weight in (.10, .25, .40):
        finalists[f"incumbent{int((1-weight)*100)}_kyrgyz{int(weight*100)}"] = combine_causal(
    ...
    overlay_score[currencies == "KGS"] = raw[selected_raw][currencies == "KGS"]
    finalists["incumbent_kgs_local_overlay"] = _outputs(
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "CN", "fixed_policy": POLICY,
        "source": "National Bank of Kyrgyzstan official daily XLS archive",
        "source_url": "https://www.nbkr.kg/EXCEL/dailyrus.xls",
        "source_file": str(Path("data/external_kyrgyzstan_nbkr_daily_2010_2026.xls")),
        ...
        "asof_rule": "local-NBKR effective date strictly before signal date; CBR date <= signal date",
