"""Packet-DW: paired multi-horizon audit of online spot-regime weighting."""
...
OUT = Path("results/research/round6/spot_online_paired_audit")
SOURCE = Path("results/research/round6/spot_online_weighting/outputs.pkl")
ONLINE = "online_global_w250_eta5"
CANDIDATES = ("noon_consensus", ONLINE, "matched_stale20")
COMPARISONS = (
    ("online_vs_noon", "noon_consensus", ONLINE),
    ("fresh_vs_stale", "matched_stale20", ONLINE),
)
...
def main() -> None:
    ...
    for hypothesis, control, challenger in COMPARISONS:
        for h in HORIZONS:
            ...
            for metric in ("lift", "symmetric", "future"):
                ...
                item[f"{metric}_difference_ci_low"] = float(np.quantile(delta, .025))
                item[f"{metric}_difference_ci_high"] = float(np.quantile(delta, .975))
    ...
    aggregate.to_csv(OUT / "paired_minimum_mean_audit.csv", index=False)

    standard = pd.read_csv(
        "results/research/round6/spot_online_weighting/standard_h5_results.csv"
    )
    selected = standard[
        (standard.candidate == ONLINE)
    ...
    pair = aggregate[aggregate.hypothesis == "online_vs_noon"].set_index("aggregate")
    selected_points = audit[audit.hypothesis == "online_vs_noon"]
    gates = {
        "point_minimum_lift_above_noon": bool(
            pair.loc["minimum", "challenger_lift"]
            > pair.loc["minimum", "control_lift"]
        ),
        "paired_minimum_lift_lower_above_zero": bool(
            pair.loc["minimum", "lift_difference_ci_low"] > 0.0
        ),
        "paired_mean_lift_lower_above_zero": bool(
            pair.loc["mean", "lift_difference_ci_low"] > 0.0
        ),
        "annual_h5_lift_at_least_1p30": bool(selected.lift.ge(1.30).all()),
        "annual_rate_between_1_and_2": bool(selected.frequency.between(1, 2).all()),
    ...
    gates["scorecard_promoted"] = bool(all(gates.values()))
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "DW", "fixed_policy": POLICY,
        "candidate": "packet-DV online_global_w250_eta5",
