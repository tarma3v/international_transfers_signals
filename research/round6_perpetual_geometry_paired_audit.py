"""Packet-DJ: paired all-horizon audit of the futures agreement geometry."""
...
OUT = Path("results/research/round6/perpetual_geometry_paired_audit")
SOURCE = Path("results/research/round6/perpetual_expert_geometry/outputs.pkl")
CANDIDATES = ("incumbent", "selected", "matched_stale20")
COMPARISONS = (
    ("geometry_vs_incumbent", "incumbent", "selected"),
...
def main() -> None:
    ...
    for hypothesis, control, challenger in COMPARISONS:
        for h in HORIZONS:
            item = {"hypothesis": hypothesis, "horizon": h,
                    "control": control, "challenger": challenger}
            for metric in ("lift", "symmetric", "future"):
                delta = (
                    draws[h][challenger][metric] - draws[h][control][metric]
                )
    ...
    standard = pd.read_csv(
        "results/research/round6/perpetual_expert_geometry/standard_h5_results.csv"
    )
    selected = standard[
