"""AP41-E: frequency-protected AP40 optimal-stopping router."""
...
from research.after_publication_ap40_effective import (
    CANDIDATE as AP40_CANDIDATE,
    FROZEN_CONTROLS,
)
from research.after_publication_ap40_effective_models import optimal_stopping_router


OUT = Path('results/research/after_publication/ap41_effective')
BASE = OUT.parent / 'ap40_effective'
UNKNOWN_H = (3, 5, 10, 20)
RATE_FLOOR = 1.10
CANDIDATE = 'weekly_optimal_stopping_logit_rate110_cap2'
CONTROLS = (AP40_CANDIDATE, *FROZEN_CONTROLS)


def build_policies(panel, old):
    ...
    routed = optimal_stopping_router(
        core, fallback, old['take_probability'],
...
def main():
    ...
    outcomes = {key: old[key] for h in HORIZONS
                for kind in ('y', 'sym', 'forward', 'floor')
                for key in (kind + str(h),)}
    ...
    sources = [
        ...
        Path('research/after_publication_ap41_effective_registered.md'),
    ]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP41-E',
        'model': 'AP40 logistic weekly optimal stopping with rate 1.10 guard',
        'rate_floor': RATE_FLOOR,
        'n_fresh_models': 0,
        'n_fresh_policies': 1,
        'selected': selected,
