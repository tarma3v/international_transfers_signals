"""AP14-E: light causal cadence repair over frozen strong predictors."""
...
from research.after_publication_ap13_effective_models import reserve_policy
...
OUT = Path('results/research/after_publication/ap14_effective')
BASE = OUT.parent / 'ap13_effective'
...
ROUTER_CONTROL = 'router_extra_local_month24_cap2'
...
FROZEN_CONTROLS = (
    EXTRA_CONTROL, ROLLING_CONTROL, LOCAL_CONTROL, RESERVE_CONTROL,
    ROUTER_CONTROL, ANCHOR, HAZARD, AP1_CAP,
)


def build_policies(panel, scores, reserve, eligible, old):
    ...
    for score_name in SCORE_NAMES:
        for kind in POLICY_KINDS:
            ...
            result = light_cadence_policy(
                scores[score_name], reserve, dates, currencies, eligible, kind)
    ...
    assert len(signals) == len(SCORE_NAMES) * len(POLICY_KINDS) + len(FROZEN_CONTROLS) + 1
...
def select_early(panel, outcomes, signals, early, groups):
    ...
    fresh = [key for key in summary.index
             if key not in FROZEN_CONTROLS and key != SIMPLE]
    ...
    if len(passing):
        ...
        passing = passing[passing.rate_cap_pass].sort_values(
            ['min_lift', 'mean_lift', 'calendar_gap_max'],
            ascending=[False, False, True], kind='stable')
        selected = str(passing.index[0]) if len(passing) else ROLLING_CONTROL
    selection = {
        'selected': selected,
...
def addition_stats(panel, scores, reserve, eligible, signals, diagnostics, old, early, later):
    dates = panel.date.to_numpy()
    currencies = panel.currency.to_numpy()
    rows = []
    for score_name in SCORE_NAMES:
        frozen_key = {
            'extra_roll2': ROLLING_CONTROL,
            'local_extra': LOCAL_CONTROL,
            'extra_ap12': EXTRA_CONTROL,
        }.get(score_name)
        baseline = (old['signal__' + frozen_key] if frozen_key else reserve_policy(
            scores[score_name], reserve, dates, currencies, eligible, 'primary'))
        for kind in POLICY_KINDS:
            key = f'{score_name}_{kind}_cap2'
            for period, scope in (('early', early), ('later', later)):
                new_signal, old_signal = signals[key] & scope, baseline & scope
                rows.append({
                    'candidate': key, 'score': score_name, 'policy': kind,
                    'period': period, 'baseline_signals': int(old_signal.sum()),
                    'signals': int(new_signal.sum()),
                    'added_vs_top30': int((new_signal & ~old_signal).sum()),
                    'removed_vs_top30': int((old_signal & ~new_signal).sum()),
                    'net_vs_top30': int(new_signal.sum() - old_signal.sum()),
                    'reserve_reason_signals': int(
                        (scope & (diagnostics[key]['reason'] == 2)).sum()),
                })
...
def main():
    ...
    # Selection is written before the later scorecard is constructed.
    ...
    addition_stats(panel, scores, reserve, eligible, signals, diagnostics, old,
                   old['early'], old['later']).to_csv(OUT / 'signal_additions.csv', index=False)

    arrays = {key: old[key] for key in ('dates', 'currencies', 'early', 'later', 'groups',
                                        'eligible_next')}
    arrays.update({key: value for key, value in outcomes.items()
                   if not key.startswith('mature')})
    arrays.update({f'score__{key}': value for key, value in scores.items()})
    ...
    sources = [DATA, BASE / 'metadata.json', BASE / 'outputs.npz',
               ...
               Path('research/after_publication_ap14_effective_registered.md')]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP14-E', 'n_rows': len(panel),
        'n_fresh_scores': len(SCORE_NAMES),
        'n_fresh_policies': len(SCORE_NAMES) * len(POLICY_KINDS),
