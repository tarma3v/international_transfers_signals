"""Independent AP32 decision-router, prefix and uncertainty audit."""
...
def main():
    ...
    bad_signals, bad_detail = build_policies(
        panel, bad_core, bad_fallback, eligible, old, ap27)
    ...
    sensitivity = pd.concat([
        ...
        for control in (AP31, AP30, AP29, AP27_STRICT, AP26_BEST, AP23_BEST,
                        AP21_STRICT)
        for block in (20, 50)
    ])
    ...
    counts = pd.DataFrame({
        'reason': ('none', 'ap26_core', 'ap23_fallback'),
        'count': [int((scope & (reason == value)).sum()) for value in range(3)],
    })
    ...
    (OUT / 'audit_checks.json').write_text(json.dumps({
        ...
        'meta_trailing_rate_reason_and_weekly_cap_rebuilt': True,
