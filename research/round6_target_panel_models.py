"""Packet-AC models on a rich causal panel of the five target currencies."""
...
from research.round6_resolved_models import (
    Spec, _bootstrap, _breakdown, _choose, _evaluate, _policy_rows,
    _row_policy, prequential_scores,
)
from research.round6_target_panel_features import (
    causality_check, load_target_panel_features,
)


OUT = Path("results/research/round6/target_panel")
...
def specs():
    return [
        Spec("target_panel_hist", "hist", "full"),
        Spec("target_panel_extra", "extra", "full"),
        Spec("target_panel_xgb", "xgb", "full"),
        Spec("target_panel_only_extra", "extra", "panel"),
    ]
...
def main() -> None:
    ...
    panel, panel_names = load_target_panel_features(index, series)
    causality_check(index, series)
    ...
    core_trusted = np.column_stack([X[:, _core_columns(names)], external[:, trusted_columns]])
    matrices = {
        "panel": np.column_stack([core_trusted, panel]),
        "full": np.column_stack([core_trusted, broad, panel]),
    }
    ...
    with RANKER_SOURCE.open("rb") as handle:
        benefit_ranker = pickle.load(handle)["rank_pair_benefit_compact_quarter_anchor25"]
    ...
    for name, raw in raw_outputs.items():
        ...
        outputs[f"{name}_benefit50"] = combine_causal(
            [raw, benefit_ranker], (.50, .50), dates, currencies,
        )
    ...
    if not chronology_ok:
        raise AssertionError("target-panel training chronology failed")
    (OUT / "protocol.json").write_text(json.dumps({
        "feature_count": len(panel_names), "feature_names": panel_names,
        "broad_feature_count": len(broad_names),
        ...
        "current_same_date_target_rates_allowed": True,
