"""Frozen-2024 weekly stabilization policy for the packet-G challenger."""
...
OUT = Path("results/research/round6/broad_cbr_weekly")
SOURCE = Path("results/research/round6/broad_cbr")
CANDIDATE = "broad_full_extra"
...
def main() -> None:
    ...
    if feasible.empty:
        raise RuntimeError("no frozen-2024 feasible weekly policy")
    ...
    for period, years in (
        ...
        item.update({"period": period, "candidate": CANDIDATE, **policy.__dict__})
        rows.append(item)
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        ...
        "policy_family": "predeclared_packet_a_weekly_controllers",
        "selection_objective": (
            "max min(overall_lift, corridor_lift_min), then overall_lift, "
            "then quarter_frequency_min"
        ),
