"""Packet-AI fixed causal-rank consensus of complementary CNY models."""
...
OUT = Path("results/research/round6/cny_consensus")
SOURCE = Path("results/research/round6/cny_explainable/outputs.pkl")
ORDER = (
    "logit50_hist50", "logit67_hist33", "logit50_extra50",
    "logit67_extra33", "logit_hist_extra_equal",
)
...
def main() -> None:
    ...
    logit = source["market_anchor_logit"]
    hist = source["market_anchor_hist"]
    extra = source["cny_intraday_extra"]
    definitions = {
        "logit50_hist50": ([logit, hist], (.50, .50)),
        "logit67_hist33": ([logit, hist], (2 / 3, 1 / 3)),
        "logit50_extra50": ([logit, extra], (.50, .50)),
        "logit67_extra33": ([logit, extra], (2 / 3, 1 / 3)),
        "logit_hist_extra_equal": ([logit, hist, extra], (1 / 3, 1 / 3, 1 / 3)),
    }
    outputs = {
        name: combine_causal(parts, weights, dates, currencies)
        for name, (parts, weights) in definitions.items()
    }
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "AI", "variants": ORDER, "fixed_policy": POLICY,
        "combination": "fixed causal per-currency score ranks",
        "weights_fitted": False, "next_rate_feature": False,
