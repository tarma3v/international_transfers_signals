"""AP35-E: monotone distributional residual CatBoost in the AP33 router."""
...
OUT = Path('results/research/after_publication/ap35_effective')
BASE = OUT.parent / 'ap34_effective'
AP26 = OUT.parent / 'ap26_effective'
AP13 = OUT.parent / 'ap13_effective'
FEATURE_META = OUT.parent / 'ap8' / 'metadata.json'
UNKNOWN_H = (3, 5, 10, 20)
CANDIDATE = 'distributional_cat_core_calendar_fallback_cap2'
FROZEN_CONTROLS = (
    AP33_CANDIDATE, AP34_CANDIDATE, AP32_CANDIDATE,
    AP23_BEST, AP26_BEST, AP21_STRICT,
)
DROP_FEATURES = {'known_change', 'known_change_z', 'local_minus_common'}
...
def fit_all(panel, X, target, cap, eligible, known_change):
    ...
    for origin in origins:
        ...
        value, stats = fit_distributional_cat(
            X, target, known_change, train, query, dates, origin)
        score[query] = value
        logs.append({
            'origin': str(origin),
        ...
        print(f'AP35 {origin}: shared {shared.sum()}, train {train.sum()}, '
              f'expanded {stats["n_augmented"]}', flush=True)
    return score, pd.DataFrame(logs)
...
def main():
    ...
    arrays.update({
        'features': X,
        'model_feature_indices': model_indices,
    ...
    sources = [
        ...
        *(folder / name for folder in (BASE, AP26, AP13)
        ...
        Path('research/after_publication_ap35_effective_registered.md'),
    ]
    (OUT / 'metadata.json').write_text(json.dumps({
        ...
        'model_feature_names': [names[i] for i in model_indices],
        'n_fresh_scores': 1,
        'n_fresh_policies': 1,
