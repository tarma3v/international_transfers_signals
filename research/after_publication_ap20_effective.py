"""AP20-E: causal geometry of AP18 and CatBoost utility experts."""
...
OUT = Path('results/research/after_publication/ap20_effective')
BASE = OUT.parent / 'ap19_effective'
AP18 = OUT.parent / 'ap18_effective'
AP12 = OUT.parent / 'ap12_effective'
UNKNOWN_H = (3, 5, 10, 20)
AP19_CAT = 'cat_mean_utility_pace365_p55_r70_month24_cap2'
...
FROZEN_CONTROLS = (
    AP19_CAT, AP18_CONTROL, AP17_CONTROL, AP12_CONTROL, ROLLING_CONTROL,
    LOCAL_CONTROL, RESERVE_CONTROL, STRICT_CONTROL, NEAR_CONTROL, ANCHOR,
    HAZARD, AP1_CAP,
)
...
def build_policies(panel, scores, reserve, eligible, old):
    ...
    for score_name in SCORE_NAMES:
        ...
        result = paced_month_policy(
            scores[score_name], reserve, dates, currencies, eligible)
...
def main():
    ...
    ap18_score = ap18['prediction__full_recent50']
    cat_score = old['prediction__cat_mean_utility']
    scores, left_rank, right_rank = expert_scores(
        ap18_score, cat_score, panel.currency.to_numpy())
    ...
    arrays['expert__ap18'] = ap18_score
    arrays['expert__cat_utility'] = cat_score
    arrays['rank__ap18'] = left_rank
    arrays['rank__cat_utility'] = right_rank
    ...
    sources = [DATA, BASE / 'metadata.json', BASE / 'outputs.npz',
               AP18 / 'metadata.json', AP18 / 'outputs.npz',
               AP12 / 'metadata.json', AP12 / 'outputs.npz',
               Path('research/after_publication_ap20_effective_registered.md')]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP20-E', 'n_rows': len(panel),
