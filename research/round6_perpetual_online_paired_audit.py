"""Packet-DL: paired audit of delayed MOEX-perpetual expert weighting."""
...
OUT = Path("results/research/round6/perpetual_online_paired_audit")
SOURCE = Path("results/research/round6/perpetual_online_weighting/outputs.pkl")
CANDIDATES = ("incumbent", "best_online", "best_online_stale20")
COMPARISONS = (
    ("online_vs_incumbent", "incumbent", "best_online"),
    ("fresh_vs_stale", "best_online_stale20", "best_online"),
)
...
def main() -> None:
    ...
    for h in HORIZONS:
        ...
        for candidate in CANDIDATES:
            ...
            draws[h][candidate] = {
                "lift": lift_draw, "symmetric": sym_draw, "future": fwd_draw,
            }
            active = valid & fired
    ...
    for hypothesis, control, challenger in COMPARISONS:
        for h in HORIZONS:
            item = {
                "hypothesis": hypothesis, "horizon": h,
    ...
    for hypothesis, control, challenger in COMPARISONS:
        for aggregate in ("minimum", "mean"):
            ...
            minimax_rows.append({
                "hypothesis": hypothesis, "aggregate": aggregate,
                "control": control, "challenger": challenger,
    ...
    minimax = pd.DataFrame(minimax_rows)
    minimax.to_csv(OUT / "paired_minimum_mean_audit.csv", index=False)

    standard = pd.read_csv(
        "results/research/round6/perpetual_online_weighting/standard_h5_results.csv"
    )
    candidate = standard[
        (standard.candidate == "best_online")
        & standard.period.isin(("retrospective_2025", "retrospective_2026"))
    ]
    candidate_combined = standard[
        (standard.candidate == "best_online")
        & (standard.period == "combined_2025_2026")
    ].iloc[0]
    min_pair = minimax[
        (minimax.hypothesis == "online_vs_incumbent")
        & (minimax["aggregate"] == "minimum")
    ].iloc[0]
    candidate_points = audit[audit.hypothesis == "online_vs_incumbent"]
    gates = {
        "point_minimum_lift_above_incumbent": bool(
            min_pair.challenger_lift > min_pair.control_lift
        ),
        "paired_minimum_lift_lower_above_zero": bool(
            min_pair.lift_difference_ci_low > 0.0
        ),
        "annual_h5_lift_at_least_1p30": bool(candidate.lift.ge(1.30).all()),
        "annual_rate_between_1_and_2": bool(candidate.frequency.between(1, 2).all()),
        "minimum_currency_h5_lift_at_least_1p30": bool(
            candidate_combined.corridor_lift_min >= 1.30
        ),
        "all_five_symmetric_benefits_positive": bool(
            candidate_points.challenger_symmetric.gt(0).all()
        ),
        "all_five_future_benefits_positive": bool(
            candidate_points.challenger_future.gt(0).all()
        ),
    }
    gates["robust_minimum_promoted"] = bool(all(gates.values()))
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "DL", "fixed_policy": POLICY,
        "candidate": "online_global_w250_eta5 selected within online family on 2024",
        "comparisons": COMPARISONS, "horizons": HORIZONS,
        "bootstrap_draws": B,
        "bootstrap": "paired moving four-week blocks, same seed/draws across horizons",
        "multiplicity": "Holm separately for lift, symmetric and future differences across 10 horizon comparisons",
        "promotion_gates": gates,
        "model_refit": False, "next_cbr_rate_used": False,
    }, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    print(audit.to_string(index=False))
    print("\nMINIMUM/MEAN\n" + minimax.to_string(index=False))
