"""Independent T7 reconstruction and market/label prefix audit."""
...
from research.temperature_t7_after_receipt_market import (
    BASE,
    CLOCKS,
    HORIZONS,
    OUT,
...
def main():
    ...
    effective = build_outcomes(series, panel, 'effective')
    dates = panel.date.to_numpy()
    currencies = panel.currency.to_numpy()
    history, _digest = load_spot_1530_history()
    known = (panel.current_price.to_numpy()
    ...
    np.testing.assert_array_equal(known, effective['y1'])
    np.testing.assert_allclose(known_bps, effective['forward1'])
    same(saved['known_probability_h1'], known)
    same(saved['known_future_bps_h1'], known_bps)
    ...
    checks = {
        ...
        'h1_known_formula_matches_target_exactly': True,
