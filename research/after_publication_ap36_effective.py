"""AP36-E: mature-only Brier Hedge inside the frozen AP33 router."""
...
from research.after_publication_ap36_effective_models import EXPERTS, mature_brier_hedge
from research.after_publication_panel import build_outcomes


OUT = Path('results/research/after_publication/ap36_effective')
BASE = OUT.parent / 'ap35_effective'
AP34 = OUT.parent / 'ap34_effective'
AP26 = OUT.parent / 'ap26_effective'
UNKNOWN_H = (3, 5, 10, 20)
CANDIDATE = 'mature_brier_hedge_core_calendar_fallback_cap2'
FROZEN_CONTROLS = (
    AP33_CANDIDATE, AP34_CANDIDATE, AP35_CANDIDATE, AP32_CANDIDATE,
...
def main():
    ...
    hedge = mature_brier_hedge(
        experts, outcomes['y20'], cap['mature20'], panel.date.to_numpy(),
    ...
    arrays['hedge_score'] = score
    for j, name in enumerate(EXPERTS):
        arrays['expert__' + name] = experts[name]
        arrays['rank__' + name] = ranks[:, j]
        arrays['weight__' + name] = weights[:, j]
        arrays['loss__' + name] = losses[:, j]
        arrays['local_count__' + name] = local_count[:, j]
        arrays['global_count__' + name] = global_count[:, j]
    arrays.update({
    ...
    sources = [
        DATA,
        *(folder / name for folder in (BASE, AP34, AP26)
          for name in ('metadata.json', 'outputs.npz')),
        Path('research/after_publication_ap36_effective_registered.md'),
    ]
