"""Independent AP34 model-refit, nested-policy and corruption audit."""
...
from research.after_publication_ap34_effective import (
    AP13,
    AP21_STRICT,
    AP23_BEST,
    AP26,
...
def main():
    ...
    bad_X[cut:] = bad_X[cut:] * -17 + 123
    bad_target[cut:] = np.where(np.isfinite(bad_target[cut:]),
                                bad_target[cut:] * -11 + 777, np.nan)
    bad_known[cut:] = bad_known[cut:] * -5 + 321
    bad_score, bad_model_detail, _ = fit_all(
        panel, bad_X, bad_target, cap, eligible, bad_known)
    same(score[:cut], bad_score[:cut])
    for name in model_detail:
        same(model_detail[name][:cut], bad_model_detail[name][:cut])
    bad_fallback = fallback.copy()
    bad_fallback[cut:] = ~bad_fallback[cut:]
    bad_signals, bad_policy_detail = build_policies(
    ...
    for name in policy_detail:
        same(policy_detail[name][:cut], bad_policy_detail[name][:cut])
    ...
    sensitivity = pd.concat([
        ...
        for control in (AP33_CANDIDATE, AP32_CANDIDATE, AP23_BEST,
    ...
    source_names = ('none', 'rolling_primary', 'residual_pace', 'month_rescue')
    counts = pd.concat([
        pd.DataFrame({
            'level': level,
            'reason': names,
    ...
    (OUT / 'audit_checks.json').write_text(json.dumps({
        ...
        'all_17_ridge_residual_survival_fits_refit_exact': True,
        'train_only_clipping_local_state_and_empirical_cdf_exact': True,
