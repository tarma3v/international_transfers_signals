"""Packet-BJ paired audit of the causal resolved-error CNY regime stack."""
...
OUT = Path("results/research/round6/cny_error_regime_paired_audit")
SOURCE = Path("results/research/round6/cny_error_regime/outputs.pkl")
SOURCE_RESULTS = Path("results/research/round6/cny_error_regime/matched_results.csv")
...
COMPARISONS = (
    ("regime_freshness", "regime_hist_stale20", "regime_hist"),
    ("logit_stack_increment", "logit50_extra50", "primary75_regime_logit25"),
    ("hist_stack_increment", "logit50_extra50", "primary75_regime_hist25"),
)
...
def main() -> None:
    ...
    decisions = {}
    for blend, hypothesis in (
        ("primary75_regime_logit25", "logit_stack_increment"),
        ("primary75_regime_hist25", "hist_stack_increment"),
    ):
        ...
        decisions[blend] = gates
    fresh_pair = summary[summary.hypothesis == "regime_freshness"].iloc[0]
    (OUT / "protocol.json").write_text(json.dumps({
        ...
        "regime_freshness_supported": bool(fresh_pair.lift_difference_ci_low > 0),
        "blend_promotion_gates": decisions,
        "model_refit": False,
