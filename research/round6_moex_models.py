"""Packet-AE quarterly models with strictly lagged official MOEX FX data."""
...
from research.round6_moex_features import build_moex_features, causality_check, load_moex_history
...
def specs():
    return [
        Spec("moex_hist", "hist", "full"),
        Spec("moex_extra", "extra", "full"),
        Spec("moex_xgb", "xgb", "full"),
    ]
...
def main() -> None:
    ...
    matrix = np.column_stack([
        X[:, _core_columns(names)], external[:, trusted_columns], broad, moex,
    ])
    ...
    for spec in specs():
        score, logs = prequential_scores(spec, matrix, y, dates, reach)
    ...
    if not chronology_ok:
        raise AssertionError("MOEX training chronology failed")
    data_manifest = json.loads((OUT / "data_manifest.json").read_text(encoding="utf-8"))
    (OUT / "protocol.json").write_text(json.dumps({
        "source": "Moscow Exchange public ISS history",
        "source_payload_sha256": payload_digest,
        "source_manifest_sha256": data_manifest["payload_sha256"],
        "asof_rule": "TRADEDATE < signal_date",
        "same_day_close_allowed": False,
        "moex_feature_count": len(moex_names), "moex_feature_names": moex_names,
        "broad_feature_count": len(broad_names), "matrix_shape": list(matrix.shape),
