"""Packet-BF paired audit of CNY waveform freshness and incremental value."""
...
OUT = Path("results/research/round6/cny_waveform_paired_audit")
SOURCE = Path("results/research/round6/cny_waveform/outputs.pkl")
SOURCE_RESULTS = Path("results/research/round6/cny_waveform/matched_results.csv")
...
COMPARISONS = (
    ("waveform_freshness", "wave_extra_stale20", "wave_extra"),
    ("primary_increment", "logit50_extra50", "primary75_wave_logit25"),
)
...
def main() -> None:
    ...
    waveform = _load(SOURCE)
    primary = _load(PRIMARY)
    outputs = {**waveform, "logit50_extra50": primary["logit50_extra50"]}
    ...
    fresh_pair = summary[summary.hypothesis == "waveform_freshness"].iloc[0]
    ...
    promotion_gates["all_promotion_gates"] = bool(all(promotion_gates.values()))
    (OUT / "protocol.json").write_text(json.dumps({
        ...
        "waveform_freshness_supported": freshness_supported,
        "blend_promotion_gates": promotion_gates,
        "model_refit": False,
