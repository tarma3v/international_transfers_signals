"""Packet-BP paired audit of the unsupervised-state correction."""
...
OUT = Path("results/research/round6/cny_unsupervised_states_paired_audit")
SOURCE = Path("results/research/round6/cny_unsupervised_states/outputs.pkl")
RESULTS = Path("results/research/round6/cny_unsupervised_states/matched_results.csv")
...
COMPARISONS = (
    ("state_freshness", "cluster_stale20_hit_lcb", "cluster_shrunk_hit_lcb"),
    ("primary_increment", "logit50_extra50", "primary75_cluster25"),
)
...
def main() -> None:
    ...
    for hypothesis, control, challenger in COMPARISONS:
        ...
        challenger_lift = float(
            y[valid & fired_challenger].mean() / y[valid].mean()
        )
        ...
        rows.append({
            ...
            "benefit_difference_ci_low": float(
                np.quantile(finite_benefit, .025)
            ),
            "benefit_difference_ci_high": float(
                np.quantile(finite_benefit, .975)
            ),
    ...
    yearly = metrics[
        (metrics.candidate == "primary75_cluster25")
    ...
    state_pair = summary[summary.hypothesis == "state_freshness"].iloc[0]
    ...
    gates = {
        ...
        "combined_lift_above_primary": bool(
            combined.lift > primary_combined.lift
        ),
        "combined_benefit_above_primary": bool(
            combined.forward_benefit_bps
            > primary_combined.forward_benefit_bps
        ),
        "paired_lift_ci_excludes_zero": bool(
            blend_pair.lift_difference_ci_low > 0
        ),
        "paired_benefit_ci_excludes_zero": bool(
            blend_pair.benefit_difference_ci_low > 0
        ),
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        ...
        "multiplicity": "Holm adjustment across two predeclared hypotheses",
        "circular_shift": "separate fixed-comparison calendar diagnostic",
        "state_freshness_supported": bool(
            state_pair.lift_difference_ci_low > 0
        ),
