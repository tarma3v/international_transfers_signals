"""AP16-E: causal per-currency deficit pacing over frozen AP12 ExtraTrees."""
...
OUT = Path('results/research/after_publication/ap16_effective')
BASE = OUT.parent / 'ap15_effective'
AP12 = OUT.parent / 'ap12_effective'
UNKNOWN_H = (3, 5, 10, 20)
AP12_CONTROL = 'extra_h5_r30_nogap_cap2'
NEAR_CONTROL = 'extra_ap12_silence14_r80_cap2'
AP14_ADAPTIVE = 'extra_ap12_adaptive105_cap2'
...
FROZEN_CONTROLS = (
    AP12_CONTROL, NEAR_CONTROL, AP14_ADAPTIVE, STRICT_CONTROL,
...
def build_policies(panel, score, reserve, eligible, old):
    ...
    for kind in POLICY_KINDS:
        key = fresh_key(kind)
        result = paced_policy(score, reserve, dates, currencies, eligible, kind)
        signals[key] = result[0]
        diagnostics[key] = {
            'primary_rank': result[1], 'reserve_rank': result[2],
            'trailing_rate': result[3], 'threshold': result[4], 'reason': result[5],
        }
        ...
        for baseline_name, baseline in (
            ...
            for period, scope in (('early', early), ('later', later)):
                ...
                rows.append({
                    ...
                    'primary_reason': int((scope & (reason == 1)).sum()),
                    'pace_reason': int((scope & (reason == 2)).sum()),
                    'month_reason': int((scope & (reason == 3)).sum()),
                })
...
def main():
    ...
    sources = [DATA, BASE / 'metadata.json', BASE / 'outputs.npz',
               ...
               Path('research/after_publication_ap16_effective_registered.md')]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP16-E', 'n_rows': len(panel),
