"""Independent reconstruction audit for T37 fixed h20 shrinkage."""
...
def audit():
    ...
    if metadata["shrink_weight"] != SHRINK_WEIGHT:
        raise AssertionError("shrink weight mismatch")
    if metadata["shrink_weight_grid_searched"] is not False:
        raise AssertionError("shrinkage incorrectly marked as searched")
    ...
    expected_history = _equal_logit_blend(
        rebuilt.loc[history, "identity_h20"],
        rebuilt.loc[history, "t34_probability"])
    if not rebuilt.loc[history].route_source.eq(
            "t34_cbr_history_shrink50").all():
        raise AssertionError("shrink route does not match CBR history")
    if not np.allclose(rebuilt.loc[history, CANDIDATE], expected_history,
                       atol=1e-12):
        raise AssertionError("fixed half-logit formula mismatch")
    ...
    if not np.allclose(rebuilt.loc[receipt, CANDIDATE],
                       rebuilt.loc[receipt, "rank_correction_selected"],
                       atol=0.0):
        raise AssertionError("receipt probability changed")
    ...
    future_route = _build_route(t22, future_t34).sort_values(key).reset_index(
        drop=True)
    ...
    if set(component.route_source) != {
            "t34_cbr_history_shrink50", "t22_after_receipt_replay"}:
    ...
    result = {
        ...
        "fixed_half_logit_shrink_rebuilt": True,
