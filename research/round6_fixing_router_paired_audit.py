"""Packet-EE: paired multi-horizon audit of the 15:30 availability router."""
...
OUT = Path("results/research/round6/fixing_router_paired_audit")
...
CANDIDATES = (
    "noon_consensus", "fixing_basis", "availability_route", "matched_stale20",
)
COMPARISONS = (
    ("route_vs_noon", "noon_consensus", "availability_route"),
    ("route_vs_fixing", "fixing_basis", "availability_route"),
    ("fresh_vs_stale", "matched_stale20", "availability_route"),
)
...
def main() -> None:
    ...
    selected_years = standard[
        (standard.candidate == "availability_route")
    ...
    minimum = aggregates[
        (aggregates.hypothesis == "route_vs_noon")
    ...
    selected_points = audit[audit.hypothesis == "route_vs_noon"]
    gates = {
    ...
    aggregate_summary = {
        hypothesis: {
            aggregate: {
                key: float(row[key])
                for key in (
                    "lift_difference", "lift_difference_ci_low",
                    "lift_difference_ci_high",
                )
            }
            for aggregate, row in block.set_index("aggregate").iterrows()
        }
        for hypothesis, block in aggregates.groupby("hypothesis")
    }
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "EE", "fixed_policy": POLICY,
        "candidate": "packet-ED availability_route",
