"""AP47-E: AP46 joint-survival score with the registered 1.20 pace buffer."""
...
OUT = Path('results/research/after_publication/ap47_effective')
BASE = OUT.parent / 'ap46_effective'
UNKNOWN_H = (3, 5, 10, 20)
RATE_FLOOR = 1.20
CANDIDATE = 'joint_allh_cat_rate120_ap37_fallback_cap2'
CONTROLS = (AP37_CANDIDATE, AP40_CANDIDATE, AP46_CANDIDATE, AP45_CANDIDATE)
...
def build_policies(panel, old):
    ...
    routed = optimal_stopping_router(
        ...
        old['input__fallback_signal'].astype(bool),
        old['joint_probability'].astype(float),
...
def main():
    ...
    sources = [
        ...
        *(BASE / name for name in ('metadata.json', 'outputs.npz')),
        Path('research/after_publication_ap47_effective_registered.md'),
    ]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP47-E',
        'model': 'frozen AP46 joint probability with AP45 rate-1.20 buffer',
        'rate_floor': RATE_FLOOR,
        'n_fresh_models': 0,
        'n_fresh_policies': 1,
        'selected': selected,
