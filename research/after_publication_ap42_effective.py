"""AP42-E: causal specialist substitution between AP26 and AP23."""
...
from research.after_publication_ap41_effective import CANDIDATE as AP41_CANDIDATE
from research.after_publication_ap42_effective_models import (
    specialist_substitution_router,
)


OUT = Path('results/research/after_publication/ap42_effective')
BASE = OUT.parent / 'ap41_effective'
AP38 = OUT.parent / 'ap38_effective'
UNKNOWN_H = (3, 5, 10, 20)
CANDIDATE = 'ap26_quality_ap23_substitution_rate105_cap2'
CONTROLS = (AP37_CANDIDATE, AP38_CANDIDATE, AP40_CANDIDATE, AP41_CANDIDATE)
...
def build_policies(panel, old, ap38):
    ...
    core_quality = ap38['core__quality_gate'].astype(bool)
    routed = specialist_substitution_router(
        core, fallback, core_quality, panel.date.to_numpy(),
...
def main():
    ...
    arrays['input__core_quality'] = ap38['core__quality_gate'].astype(bool)
    ...
    sources = [
        ...
        *(folder / name for folder in (BASE, AP38)
        ...
        Path('research/after_publication_ap42_effective_registered.md'),
    ]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP42-E',
        'model': 'AP38 core quality plus AP23 rate105 substitution',
        'core_rate_floor': 1.0,
