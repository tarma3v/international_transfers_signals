"""Download frozen MOEX 10-minute spot candles for the 15:30 product."""
...
OUT = Path("results/research/round6/moex_spot_1530")
...
def fetch_ticker(ticker: str) -> dict:
    ...
    while True:
        query = urlencode({
            "from": FROM, "till": TILL, "interval": INTERVAL,
        ...
        if tuple(block["columns"]) != COLUMNS:
            raise AssertionError(f"unexpected 10-minute spot schema: {block['columns']}")
        page = block["data"]
        rows.extend(page)
    ...
    if begin != sorted(begin) or len(begin) != len(set(begin)):
        raise AssertionError(f"non-unique or unsorted 10-minute candles for {ticker}")
    for row in rows:
        if any(
            row[COLUMNS.index(name)] is None
            or float(row[COLUMNS.index(name)]) <= 0
            for name in ("open", "close", "high", "low")
        ):
            raise AssertionError(f"invalid 10-minute OHLC for {ticker}: {row}")
...
def main() -> None:
    ...
    payload = {
        ...
        "api": "MOEX ISS public 10-minute currency/SELT candles",
        "timezone": "Europe/Moscow", "from": FROM, "till": TILL,
    ...
    manifest = {
        ...
        "strict_asof_rule": "candle end < signal_date 15:30:00",
        "methodology_window": "10:00 <= transaction time < 15:30 Europe/Moscow",
        "methodology_sources": [
            "https://cbr.ru/Content/Document/File/162004/metod_6290-u.pdf",
            "https://cbr.ru/Reception/TopicalMessage/Page/2661",
        ],
