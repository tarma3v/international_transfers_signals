"""Download the frozen official MOEX hourly spot-FX candle packet."""
...
OUT = Path("results/research/round6/moex_spot_hourly")
...
def fetch_ticker(ticker: str) -> dict:
    ...
    while True:
        ...
        if tuple(block["columns"]) != COLUMNS:
            raise AssertionError(f"unexpected hourly spot schema: {block['columns']}")
        page = block["data"]
        rows.extend(page)
    ...
    if begin != sorted(begin) or len(begin) != len(set(begin)):
        raise AssertionError(f"non-unique or unsorted hourly spot candles for {ticker}")
    for row in rows:
        if any(
            ...
            raise AssertionError(f"invalid OHLC spot candle for {ticker}: {row}")
...
def main() -> None:
    ...
    payload = {
        ...
        "api": "MOEX ISS public 60-minute currency/SELT candles",
    ...
    manifest = {
        ...
        "missing_volume_value_preserved": True,
        "instruments": [{
