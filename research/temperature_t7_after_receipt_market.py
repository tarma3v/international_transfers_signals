"""T7: completed-candle corrections on top of after-receipt experts."""
...
OUT = Path('results/research/temperature/t7_after_receipt_market')
BASE = Path('results/research/after_publication/ap51_benefit')
RECEIPT = dt.time(18, 0)
CLOCKS = (dt.time(18, 30), dt.time(20, 0))
HORIZONS = (3, 5, 10, 20)
...
def market_delta(panel, history, clock):
    ...
    for row, day in enumerate(panel.date):
        start = dt.datetime.combine(day, RECEIPT)
        end = dt.datetime.combine(day, clock)
        begins, ends = item['begin'], item['end']
        ids = np.arange(
            int(np.searchsorted(begins, dt.datetime.combine(day, dt.time(7)),
                                side='left')),
            int(np.searchsorted(ends, end, side='left')), dtype=int)
        if not len(ids):
            continue
        nominal = np.array([
            begins[i] + dt.timedelta(minutes=10) for i in ids], dtype=object)
        same_day = np.array([value.date() == day for value in begins[ids]])
        complete = same_day & (ends[ids] < end) & (nominal <= end)
        ids, nominal = ids[complete], nominal[complete]
        if not len(ids):
            continue
        anchor = ids[(ends[ids] < start) & (nominal <= start)]
        later = ids[ends[ids] >= start]
        if not len(anchor) or not len(later):
            continue
        result[row] = 10000. * np.log(
            item['close'][later[-1]] / item['close'][anchor[-1]])
        available[row] = True
...
def physical_causality_check(panel, history, clock,
    ...
    original = market_delta(panel, history, clock)[0]
    changed = {}
    ...
    altered = market_delta(panel, changed, clock)[0]
...
def main():
    ...
    for clock in CLOCKS:
        ...
        delta, available = market_delta(panel, history, clock)
    ...
    sources = [
        ...
        Path('research/temperature_t7_after_receipt_market_registered.md'),
        Path('data/moex_spot_fx_10min_2022_2026.json'),
    ]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'temperature-T7',
        'invalidated': True,
        'invalidated_by': 'temperature-T7B',
        'invalidation_reason': ('AP49/AP50 base decision is 18:30 with a '
                                '20-minute market delay, not an 18:00 snapshot'),
        'receipt_clock_assumption': RECEIPT.isoformat(),
