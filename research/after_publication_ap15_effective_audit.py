"""Independent AP15 controller-state, selection and uncertainty audit."""
...
from research.after_publication_ap15_effective import (
    ADAPTIVE_CONTROL,
    ANCHOR,
    AP1_CAP,
    AP12,
...
def main():
    ...
    for candidate, detail in diagnostics.items():
        ...
        assert np.isin(detail['threshold'], [.675, .6875, .70]).all()
        month = detail['reason'] == 3
        assert all(day.day >= 24 for day in panel.date.to_numpy()[month])

    # A month rescue may only be the first signal in that currency-month.
    dates = panel.date.to_numpy()
    currencies = panel.currency.to_numpy()
    for candidate, detail in diagnostics.items():
        for i in np.flatnonzero(detail['reason'] == 3):
            earlier = ((currencies == currencies[i])
                       & np.array([(d.year, d.month) == (dates[i].year, dates[i].month)
                                   for d in dates])
                       & (np.arange(len(dates)) < i))
            assert not signals[candidate][earlier].any()
    ...
    for kind in POLICY_KINDS:
        ...
        rebuilt = targeted_policy(p2, r2, dates, currencies, eligible, kind)
        candidate = fresh_key(kind)
        original = diagnostics[candidate]
        for left, right in zip((signals[candidate], original['primary_rank'],
                                original['reserve_rank'], original['threshold'],
                                original['reason']), rebuilt):
    ...
    diagnostic = list(dict.fromkeys([selection['selected'], *fresh]))
    controls = [AP12_CONTROL, NEAR_CONTROL, ADAPTIVE_CONTROL, STRICT_CONTROL,
                SELECTED_CONTROL, ROLLING_CONTROL, LOCAL_CONTROL, RESERVE_CONTROL,
                ANCHOR, HAZARD, AP1_CAP]
    common_audit(OUT, diagnostic, [(control, diagnostic) for control in controls])
    ...
    (OUT / 'audit_checks.json').write_text(json.dumps({
        ...
        'all_five_signals_and_controller_state_rebuilt': True,
