"""Packet-EA: label-free noon/fixing consensus at 15:30 Moscow."""
...
from research.round6_spot_agreement_geometry import FORMULAS, formulas
...
OUT = Path("results/research/round6/fixing_consensus")
NOON_PATH = Path("results/research/round6/three_view_futures_consensus/outputs.pkl")
FIXING_PATH = Path("results/research/round6/spot_1530_nowcast/outputs.pkl")
RANK_WINDOW = 250
RANK_MINIMUM = 20
...
def main() -> None:
    ...
    fixing_output = _load(FIXING_PATH, "selected")
    stale_fixing_output = _load(FIXING_PATH, "matched_stale20")
    noon_score = row_scores(noon_output, len(index))
    fixing_score = row_scores(fixing_output, len(index))
    stale_fixing_score = row_scores(stale_fixing_output, len(index))
    future_rank_check(noon_score, dates, currencies)
    future_rank_check(fixing_score, dates, currencies)
    ...
    stale_fixing_rank = causal_percentiles(
        stale_fixing_score, dates, currencies, RANK_WINDOW, RANK_MINIMUM,
    )
    fresh_formula = formulas(noon_rank, fixing_rank)
    stale_formula = formulas(noon_rank, stale_fixing_rank)
    candidates = {
        "noon_consensus": noon_output,
        "fixing_basis": fixing_output,
        **{
    ...
    _circular_shift_audit(
        y5, dates, currencies, valid, masks, "fixing_consensus_h5",
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "EA", "fixed_policy": POLICY,
        "decision_time": "15:30:00 Europe/Moscow",
        "experts": (
            "packet-DO noon consensus",
            "packet-DZ CNY session-mean basis",
        ),
        ...
        "selected": selected,
        "matched_control": "fixing basis delayed 20 target rows only",
        "selection_objective": (
            "maximum worst official case-lift over h=1/3/5/10/20 "
            "with positive symmetric and future-only benefits"
        ),
