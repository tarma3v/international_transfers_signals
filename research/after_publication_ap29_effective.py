"""AP29-E: mature-competence backstop for the AP26 specialist core."""
...
from research.after_publication_ap27_effective_models import triple_paced_month_policy
from research.after_publication_panel import build_outcomes


OUT = Path('results/research/after_publication/ap29_effective')
BASE = OUT.parent / 'ap27_effective'
AP26 = OUT.parent / 'ap26_effective'
AP23 = OUT.parent / 'ap23_effective'
UNKNOWN_H = (3, 5, 10, 20)
CANDIDATE = 's200_comp95_r60_backstop_month24_cap2'
...
def policy_inputs(old, ap26, ap23):
    return {
        'primary': old['input__primary'],
        'pace': ap26['pace_score__y20_shrink200'],
        'backstop': ap23['pace_score__soft730_pace'],
...
def build_policies(panel, inputs, eligible, old):
    result = triple_paced_month_policy(
        inputs['primary'], inputs['pace'], inputs['backstop'], inputs['reserve'],
        panel.date.to_numpy(), panel.currency.to_numpy(), eligible,
        backstop_rate=.95, backstop_threshold=.60)
...
def main():
    ...
    sources = [
        DATA,
        *(folder / name for folder in (BASE, AP26, AP23)
          for name in ('metadata.json', 'outputs.npz')),
        Path('research/after_publication_ap29_effective_registered.md'),
    ]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP29-E', 'n_rows': len(panel), 'n_fresh_policies': 1,
