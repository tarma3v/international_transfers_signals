"""AP22-E: causal rolling/local consensus primary with separate cadence."""
...
OUT = Path('results/research/after_publication/ap22_effective')
BASE = OUT.parent / 'ap21_effective'
...
def candidate_key(name):
    return f'{name}_consensus_dual_month24_cap2'
...
def main():
    ...
    arrays['rank__rolling'] = rolling_rank
    arrays['rank__local'] = local_rank
    ...
    sources = [DATA, *(folder / name for folder in folders
               ...
               Path('research/after_publication_ap22_effective_registered.md')]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP22-E', 'n_rows': len(panel),
