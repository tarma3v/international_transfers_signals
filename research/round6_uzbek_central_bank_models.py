"""Packet-CL: causal shadow-rate signals from the Central Bank of Uzbekistan."""
...
from research.round6_uzbek_central_bank_features import (
    build_uzbek_cbu_features, causality_check, load_uzbek_cbu,
)


OUT = Path("results/research/round6/uzbek_central_bank_models")
...
RAW_ORDER = (
    "uzbek_cbu_direct_basis", "uzbek_cbu_negative_direct_basis",
    "uzbek_cbu_usd_basis", "uzbek_cbu_negative_usd_basis",
    "uzbek_cbu_cny_basis", "uzbek_cbu_negative_cny_basis",
    "uzbek_cbu_consensus_basis", "uzbek_cbu_negative_consensus_basis",
    "uzbek_cbu_inverse_rub_momentum_1", "uzbek_cbu_inverse_rub_momentum_2",
    "uzbek_cbu_inverse_rub_momentum_5", "uzbek_cbu_inverse_rub_momentum_10",
    "uzbek_cbu_direct_usd_disagreement", "uzbek_cbu_direct_cny_disagreement",
    "uzbek_cbu_usd_cny_disagreement", "uzbek_cbu_consensus_stale20",
)
...
def horizon_rows(outputs, years, targets, forwards, dates, currencies):
    rows = []
    for candidate, output in outputs.items():
        for h in HORIZONS:
            ...
            rows.append({
                "candidate": candidate, "horizon": h,
                "case_lift": lift, "matched_random_day_rate": base,
...
def main() -> None:
    ...
    local, digest = load_uzbek_cbu()
    matrix, names = build_uzbek_cbu_features(index, series, references, local)
    causality_check(index, series, references, local)
    col = {name: matrix[:, names.index(name)].astype(float) for name in names}
    raw = {
        "uzbek_cbu_direct_basis": col["uzbek_cbu_direct_basis_bps"],
        "uzbek_cbu_negative_direct_basis": -col["uzbek_cbu_direct_basis_bps"],
        "uzbek_cbu_usd_basis": col["uzbek_cbu_usd_basis_bps"],
        "uzbek_cbu_negative_usd_basis": -col["uzbek_cbu_usd_basis_bps"],
        "uzbek_cbu_cny_basis": col["uzbek_cbu_cny_basis_bps"],
        "uzbek_cbu_negative_cny_basis": -col["uzbek_cbu_cny_basis_bps"],
        "uzbek_cbu_consensus_basis": col["uzbek_cbu_consensus_basis_bps"],
        "uzbek_cbu_negative_consensus_basis": -col["uzbek_cbu_consensus_basis_bps"],
        "uzbek_cbu_inverse_rub_momentum_1": -col["uzbek_cbu_rub_quote_ret_1"],
        "uzbek_cbu_inverse_rub_momentum_2": -col["uzbek_cbu_rub_quote_ret_2"],
        "uzbek_cbu_inverse_rub_momentum_5": -col["uzbek_cbu_rub_quote_ret_5"],
        "uzbek_cbu_inverse_rub_momentum_10": -col["uzbek_cbu_rub_quote_ret_10"],
        "uzbek_cbu_direct_usd_disagreement": col["uzbek_cbu_direct_minus_usd_bps"],
        "uzbek_cbu_direct_cny_disagreement": col["uzbek_cbu_direct_minus_cny_bps"],
        "uzbek_cbu_usd_cny_disagreement": col["uzbek_cbu_usd_minus_cny_bps"],
        "uzbek_cbu_consensus_stale20": delayed_by_currency(
            col["uzbek_cbu_consensus_basis_bps"][:, None], index, rows=20,
        )[:, 0],
    }
    ...
    finalists = {"incumbent": incumbent, "uzbek_selected": raw_outputs[selected_raw]}
    for weight in (.10, .25, .40):
        finalists[f"incumbent{int((1-weight)*100)}_uzbek{int(weight*100)}"] = combine_causal(
    ...
    overlay_score[currencies == "UZS"] = raw[selected_raw][currencies == "UZS"]
    finalists["incumbent_uzs_local_overlay"] = _outputs(
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "CL", "fixed_policy": POLICY,
        "source": "Central Bank of Uzbekistan official yearly XLS archive",
        "source_url": "https://cbu.uz/ru/arkhiv-kursov-valyut/",
        "source_files": [str(path) for path in sorted(
            Path("data").glob("external_uzbekistan_cbu_*.xls")
        )],
        ...
        "asof_rule": "local-CB effective date strictly before signal date; CBR date <= signal date",
