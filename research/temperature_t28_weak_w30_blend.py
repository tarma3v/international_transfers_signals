"""T28: weak logit blends toward causal delayed-w30 h20 calibration."""
...
REGISTERED = Path("research/temperature_t28_weak_w30_blend_registered.md")
...
def _beta_name(beta):
    return f"blend_w30_b{int(round(beta * 100)):03d}"


BLENDS = tuple(_beta_name(beta) for beta in BETAS)
PRIORITY = {name: i for i, name in enumerate(BLENDS)}
MODELS = ("identity_early", "t25_base", "weak_selected", "w30_full", *BLENDS)


def _blend_candidates(t25, w30):
    base_logit = _logit(np.asarray(t25, dtype=float))
    correction = _logit(np.asarray(w30, dtype=float)) - base_logit
    return {
        _beta_name(beta): expit(base_logit + beta * correction)
        for beta in BETAS
    }
...
def run():
    ...
    delayed, states, feedback_details = _delayed_candidates(
        combined, combined.t25_base)
    w30 = delayed["delayed_global_w30"]
    blends = _blend_candidates(combined.t25_base, w30)
    ...
    intervals = pd.concat([
        _paired_intervals(evaluated, "weak_selected", "t25_base", "t25"),
        _paired_intervals(evaluated, "weak_selected", "identity_early", "identity"),
    ], ignore_index=True)

    primary = metrics[(metrics.model == "weak_selected") & (metrics.slice == "ALL")].iloc[0]
    year_rows = metrics[(metrics.model == "weak_selected") & (metrics.slice == "year")]
    currency_rows = metrics[(metrics.model == "weak_selected") & (metrics.slice == "currency")]
    brier_intervals = intervals[(intervals.comparison == "t25") & (intervals.metric == "brier")]
    passed = bool(
        final_model != "t25_base"
        and validation_passed
        and primary.brier_delta < 0.0
    ...
    metadata = {
        ...
        "checks": {
            ...
            "feedback_trace_causal": bool((
                states.loc[states.latest_feedback_maturity_ord.notna(),
