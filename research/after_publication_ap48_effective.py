"""AP48-E: substitute AP23 opportunities after AP46 same-week vetoes."""
...
from research.after_publication_ap47_effective import CANDIDATE as AP47_CANDIDATE
from research.after_publication_ap48_effective_models import (
    same_week_substitution_router,
)


OUT = Path('results/research/after_publication/ap48_effective')
BASE = OUT.parent / 'ap46_effective'
AP47 = OUT.parent / 'ap47_effective'
UNKNOWN_H = (3, 5, 10, 20)
CANDIDATE = 'joint_allh_ap23_sameweek_substitution_cap2'
CONTROLS = (AP37_CANDIDATE, AP40_CANDIDATE, AP46_CANDIDATE, AP47_CANDIDATE)


def build_policies(panel, old, ap47):
    routed = same_week_substitution_router(
        old['signal__' + AP46_CANDIDATE], old['policy__core_veto'],
        old['input__fallback_signal'], panel.date.to_numpy(),
        panel.currency.to_numpy(), old['eligible_next'])
    signals = {CANDIDATE: routed[0]}
    for control in CONTROLS:
        source = ap47 if control == AP47_CANDIDATE else old
        signals[control] = source['signal__' + control]
    signals[SIMPLE] = old['signal__' + SIMPLE]
    return signals, {
        'reason': routed[1],
        'pending_veto_before': routed[2],
        'week_used_before': routed[3],
        'core_veto': old['policy__core_veto'].astype(bool),
    }
...
def main():
    ...
    with np.load(AP47 / 'outputs.npz') as source:
        ap47 = {key: source[key] for key in source.files}
    ...
    sources = [
        DATA,
        *(folder / name for folder in (BASE, AP47)
          for name in ('metadata.json', 'outputs.npz')),
        Path('research/after_publication_ap48_effective_registered.md'),
    ]
    (OUT / 'metadata.json').write_text(json.dumps({
        'packet': 'AP48-E',
        'model': 'causal AP46 primary with same-week AP23 veto substitution',
        'n_fresh_models': 0,
        'n_fresh_policies': 1,
        'selected': selected,
        'reference': 'today-effective CBR',
