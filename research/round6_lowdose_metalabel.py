"""Packet-AB low-dose tail meta-label under the primary policy."""
...
OUT = Path("results/research/round6/lowdose_metalabel")
META_SOURCE = Path("results/research/round6/tail_metalabel/outputs.pkl")
PRIMARY_SOURCE = Path("results/research/round6/direct_rankers/outputs.pkl")
PRIMARY_NAME = "rank_pair_benefit_compact_quarter_anchor25"
...
def main() -> None:
    ...
    with META_SOURCE.open("rb") as handle:
        meta = pickle.load(handle)["tail50_extra"]
    with PRIMARY_SOURCE.open("rb") as handle:
        primary = pickle.load(handle)[PRIMARY_NAME]
    outputs = {}
    for weight in PRIMARY_WEIGHTS:
        name = f"primary{int(round(weight*100)):02d}_tail50_extra"
        outputs[name] = combine_causal(
            [primary, meta], (weight, 1.0 - weight), dates, currencies,
        )
    ...
    selected = results[results.period == "screen_2024"].sort_values("screen_rank")
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        "primary": PRIMARY_NAME, "meta": "tail50_extra",
