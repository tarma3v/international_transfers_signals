"""Independent AP37 mature-precision, router and prefix audit."""
...
from research.after_publication_ap37_effective import (
    AP33,
    BASE,
    CANDIDATE,
    FROZEN_CONTROLS,
...
def main():
    ...
    rebuilt = mature_support_precision(
        ranks, outcomes, cap['mature20'], dates, currencies, eligible, core)
    keys = ('support_stratum', 'quality_gate', 'overall_precision',
            'global_stratum_precision', 'local_stratum_precision',
            'overall_count', 'global_stratum_count', 'local_stratum_count')
    for key, value in zip(keys, rebuilt):
    ...
    bad_precision = mature_support_precision(
        bad_ranks, bad_outcomes, cap['mature20'], dates, currencies,
        eligible, bad_core)
    bad_signals, bad_detail = build_policies(
        panel, bad_core, bad_fallback, bad_precision[1], eligible, old)
    for actual, corrupted in zip(rebuilt, bad_precision):
    ...
    names = ('none', 'ap26_core', 'precision_late_week', 'silence10')
    counts = pd.DataFrame({
        'reason': names,
    ...
    leader = old['signal__' + AP33_CANDIDATE].astype(bool)
    comparison = pd.DataFrame({
        'change': ('same_selected', 'removed_from_ap33', 'added_vs_ap33'),
        'count': (
    ...
    (OUT / 'late_diagnostics.json').write_text(json.dumps({
        ...
        'late_decision_changes_vs_ap33': dict(zip(
    ...
    (OUT / 'audit_checks.json').write_text(json.dumps({
        ...
        'mature_all_horizon_precision_counts_and_shrinkage_exact': True,
        'precision_calendar_rate_silence_reason_and_cap_exact': True,
