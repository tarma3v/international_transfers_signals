"""Packet-DP: paired audit of the selected three-view futures consensus."""
...
OUT = Path("results/research/round6/three_view_futures_paired_audit")
SOURCE = Path("results/research/round6/three_view_futures_consensus/outputs.pkl")
CANDIDATES = ("incumbent", "selected", "matched_stale20")
COMPARISONS = (
    ("consensus_vs_incumbent", "incumbent", "selected"),
...
def main() -> None:
    ...
    standard = pd.read_csv(
        "results/research/round6/three_view_futures_consensus/standard_h5_results.csv"
    )
    selected = standard[
    ...
    min_pair = minimax[
        (minimax.hypothesis == "consensus_vs_incumbent")
        & (minimax["aggregate"] == "minimum")
    ].iloc[0]
    selected_points = audit[audit.hypothesis == "consensus_vs_incumbent"]
    gates = {
    ...
    (OUT / "protocol.json").write_text(json.dumps({
        "packet": "DP", "fixed_policy": POLICY,
        "candidate": "packet-DO selected pair_arithmetic",
