...
## T3-T8B: непрерывная температура перевода для интерфейса

Презентация из основной ветки формулирует продукт как экранный индикатор плюс
отдельное редкое уведомление. T8B причинно объединил фазовые модели в один
...
`push_now`. Ретроспективный пакет содержит **45 990** уникальных временных
...
регрессия prior не обошла и потому должна использоваться осторожно.
...
Push остаётся отдельным решением AP37 с max2/week. В T8B сохранено
...
[T8B protocol](research/temperature_t8b_unified_router_registered.md) ·
[T8B audit](results/research/temperature/t8b_unified_router/audit_checks.json) ·
[PDF checkpoint](output/pdf/ivan_continuous_temperature_checkpoint.pdf).
Полный набор после T8B: **320 тестов**, все прошли.
