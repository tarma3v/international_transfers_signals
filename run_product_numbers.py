...
def pilot_sizing(series, X, names, index, dates) -> None:
    ...
    # наблюдений. Design effect измеряем сами, отношением дисперсии среднего при
...
def base_rate_drift(index, dates) -> None:
    """Почему метрика кейса измеряет режим рынка, а не качество момента."""
    bar("ДРЕЙФ БАЗОВОЙ СТАВКИ: метрика кейса измеряет режим рубля")
    s = load()
    y = np.full(len(index), np.nan)
    for r, (c, i, _d) in enumerate(index):
        t = target_now_favourable(s[c].values, i, H)
    ...
    print("ТОГО ЖЕ периода (ml/evaluate.py::lift, аргумент scope), иначе он измеряет дрейф.")
