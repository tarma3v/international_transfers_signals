...
W("Обучение — расширяющееся окно, тест — календарный год с 2021, очистка на h дней.")
W("«Достижимая выгода» — форвардная половина метрики заказчика: то, что клиент реально может забрать.\n")
W("## Методология\n")
W("Два решения фиксируются ДО теста, иначе тест участвует в них и результат завышен:\n")
...
for tname, tlabel in (("fav", "Сейчас выгодно"), ("close", "Окно закрывается")):
    ...
    for h in HORIZONS:
        ...
        chosen, _rep = select_model(X, y, dates, 2021)
        best = {"tz": -9e9, "md": -9e9, "sel": float("nan")}
        for nm, f, is_md in rows:
            lf, _, n = lift(f, y)
            rate = f.sum() / max(int(oos.sum()), 1)
            pw = n / len(CORRIDORS) / (7.0 * 52 * (oos.sum() / len(y)))
        ...
        W(f"| {h} | *uplift к лучшему ТЗ: модель выбрана ДО теста ({chosen})* | | | | | "
          f"**{best['sel']-best['tz']:+.0f} бп** | |")
        W(f"| {h} | *справочно — максимум по тесту (результатом не является)* | | | | | "
